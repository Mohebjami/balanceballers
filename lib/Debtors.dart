...
class _DebtorsState extends State<Debtors> {
  ...
  Future<List<Map<String, String?>>> showPlayersDropDownDebt() async {
    final CollectionReference collectionReference = FirebaseFirestore.instance.collection('players');
    ...
    }).where((item) => item['Name'] != null && item['Last Name'] != null).toList();
  }
        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: (searchString == null || searchString.trim() == '') ? collectionRef.snapshots()
                  : collectionRef.where('Name', isGreaterThanOrEqualTo: searchString).snapshots(),
                      ...
                      return Card(
                        ...
                        child: Slidable(
                          ...
                          child: ListTile(
                            ...
                            trailing: Text(data['Debt'].toString(), style: const TextStyle(color: Colors.white,fontSize: 20),),
                              ...
                              Container(
                                ...
                                child: Padding(
                                  ...
                                  child: DropdownButton<Map<String, String?>>(
                                    ...
                                    dropdownColor: const Color.fromRGBO(255, 180, 0, 1.0),
                                    underline: Container(
                                    ...
                                    items: PalyerDropDown.map<DropdownMenuItem<Map<String, String?>>>((Map<String, String?> value) {
                                          return DropdownMenuItem<Map<String, String?>>(
                                            alignment: Alignment.centerRight,
                                            value: value,
                                            child: Text(value['Name']! + ' ' + value['Last Name']!),
                                          );
                                        }).toList(),
                                  ),
                                ),
                              ...
                              SizedBox(
                                ...
                                child: ElevatedButton(
                                  onPressed: () async {
                                    ...
                                    CollectionReference collRef = FirebaseFirestore.instance.collection('Debtors');
                                    // First, check if the data already exists in the Debtors table
                                    Query query = collRef.where('Name', isEqualTo: selectedPlayer?['Name']).where('Last Name', isEqualTo: selectedPlayer?['Last Name']);
                                    query.get().then((querySnapshot) async {
                                      if (querySnapshot.docs.isNotEmpty) {
                                        ...
                                        await collRef.add({
                                          ...
                                          'Last Name': selectedPlayer?['Last Name'],
                                          'Debt': dbt,
                                        }).then((value) async {
                                          Query query = FirebaseFirestore.instance.collection('players')
                                              .where('Name', isEqualTo: selectedPlayer?['Name']).where('Last Name', isEqualTo: selectedPlayer?['Last Name']);
                                          query.get().then((querySnapshot) async {
                                            if (querySnapshot.docs.isNotEmpty) {
                                              for (var doc in querySnapshot.docs) {
                                                DocumentReference docRef = doc.reference;
